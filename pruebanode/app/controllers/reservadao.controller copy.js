...
exports.getReservasDisponibles = (req, res) => {
    ...
    const exclusionValues = exclusionList.map((item) => `(${item.hora_inicial}, ${item.hora_final}, '${item.fecha}')`).join(',');
    sequelize
        .query(
        "SELECT id_mesa, fecha FROM reservas r WHERE " +
            `r.id_restaurante = ${req.params.id_res} ` +
            `AND (r.hora_inicial, r.hora_final, r.fecha) NOT IN (${exclusionValues})`
            // ` (${exclusionConditions})  ` +
            // 'AND r.id_restaurante = :id_res ',
            
        )
        .then(async (reservas) => {
            const mesasIds = reservas[0].map((reserva) => reserva.id_mesa);
            const mesas = await MesaDao.findAll({
                where: {
                    id: mesasIds
                }
            });
