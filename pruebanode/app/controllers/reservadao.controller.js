...
const Client = require("./clientedao.controller.js");
const Mesa = require("./mesadao.controller.js");
const Restaraurant = require("../controllers/restaurantedao.controller.js");
...
exports.getReservasOcupadas2 = (req, res) => {
  ...
  sequelize
    .query(
      "SELECT id_mesa, fecha FROM reservas r WHERE ( :horaInicio between r.hora_inicial and r.hora_final-1  " +
    ...
    .then((data) => {
      ...
      data[0].forEach((elem) => {
        ...
        console.log(fecha);
        fecha2 =
          elem.fecha.getFullYear() +
          "-" +
          (elem.fecha.getMonth() + 1) +
          "-" +
          elem.fecha.getDate();
        fecha3 =
          fecha.getFullYear() +
...
exports.getReservasDisponibles = async (req, res) => {
  const horasI = req.params.hora_i.split(",");
  const horasF = req.params.hora_f.split(",");
  const fechas = req.params.fecha.split(",");
  const exclusionList = horasI.map((horaInicial, index) => {
    return {
      hora_inicial: horaInicial,
      hora_final: horasF[index],
      fecha: fechas[index],
    };
  });
  const exclusionValues = exclusionList
    .map(
      (item) => `(${item.hora_inicial}, ${item.hora_final}, '${item.fecha}')`
    )
    .join(",");
  try {
    const [reservas] = await sequelize.query(
      "SELECT id_mesa, fecha FROM reservas r WHERE " +
        `r.id_restaurante = ${req.params.id_res} ` +
        `AND (r.hora_inicial, r.hora_final, r.fecha) IN (${exclusionValues})`
      // ` (${exclusionConditions})  ` +
      // 'AND r.id_restaurante = :id_res ',
    );
    const [mesas] = await sequelize.query(
        "SELECT * FROM mesas m WHERE " +
            `m.id_restaurante = ${req.params.id_res} `
    );

    const mesasDisponibles = [...mesas]
    for (const mesa of mesas) {
        for (const reserva of reservas) {
            if (mesa.id === reserva.id_mesa) {
